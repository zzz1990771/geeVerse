...
p=200
beta0=rep(1,9)
beta0=c(rep(1,5),rep(2,4))
p0=length(beta0)
beta = c(beta0,rep(0,p-p0+5))
n_obs<-rep(5,n_sub);
...
for(tau in tau_list){
  ...
  while(sim <= n_sim){
    ...
    #try({
      ...
      #screening by SIS
      SIS_result <- SIS::SIS(x,y,iter=FALSE,nsis=200)
  ...
  # compile results ------------------------------------
  final_results_sim[c(2*which(tau==tau_list)-1,2*which(tau==tau_list)),] <- compile_result_hd(PQGEE_results,SIS_results,n_sim,cutoff = 0.1,beta=beta,beta0=beta0,p=1005)
